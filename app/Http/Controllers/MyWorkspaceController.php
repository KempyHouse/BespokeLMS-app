...
/**
 * The learner "My" workspace — a per-user configurable dashboard.
 *
 * The user places widgets from the platform library onto their own dashboard and
 * arranges them (size + order); the layout is saved to user_dashboards. Which
 * widgets a user may add is governed by role: the registry is read pre-filtered
 * to the signed-in user's role (dashboard_widget_visibility). Every widget is
 * powered by live data scoped to this user (or, for the platform widgets, the
 * platform owner's estate) and shows an honest empty state until data exists —
 * nothing is fabricated.
 */
final class MyWorkspaceController extends Controller
{
    /** Widget keys whose data is personal (governed by the user's own learning). */
    ...
    public function index(Request $request, ReadsDashboards $dashboards, ReadsWidgetData $data): View
    {
        ...
        return view('my.home', [
            ...
            'personalKeys' => self::PERSONAL_KEYS,
            'registryError' => $registryError,
        ]);
    }

    /**
     * Persist the signed-in user's dashboard layout. Keys are whitelisted against
