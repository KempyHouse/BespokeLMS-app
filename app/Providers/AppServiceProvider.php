...
use App\Support\Supabase\SupabaseCourseWorkflowWriter;
...
class AppServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        ...
        $this->app->singleton(WritesCourseWorkflow::class, function (Application $app): SupabaseCourseWorkflowWriter {
            /** @var array<string,mixed> $config */
            $config = $app['config']->get('services.supabase', []);

            return new SupabaseCourseWorkflowWriter(
                $app->make(HttpFactory::class),
                (string) ($config['url'] ?? ''),
                (string) ($config['service_role_key'] ?? ''),
                (int) ($config['timeout'] ?? 10),
